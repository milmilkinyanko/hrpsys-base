...
namespace rats
{
  ...
  private:
    ...
    bool solved;
    double leg_margin[4], stride_limitation_for_circle_type[4], overwritable_stride_limitation[4], footstep_modification_gain, cp_check_margin[2], emergency_step_time[3], cp_check_margin_step[2], margin_time_ratio;
    bool use_stride_limitation, act_contact_states[4], is_emergency_walking[2], modify_footsteps, is_emergency_step;
  ...
  public:
    ...
    gait_generator (double _dt,
        ...
        velocity_mode_flg(VEL_IDLING), emergency_flg(IDLING), is_emergency_step(false), margin_time_ratio(0.01), footstep_modification_gain(5e-6),
        ...
        for (size_t i = 0; i < 4; i++) overwritable_stride_limitation[i] = 0.15;
        for (size_t i = 0; i < 4; i++) act_contact_states[i] = false;
        ...
        for (size_t i = 0; i < 2; i++) cp_check_margin_step[i] = 0.03;
    };
    ~gait_generator () {
    ...
    void set_cp_check_margin_step (const double _cp_check_margin_step[2]) {
      for (size_t i=0; i < 2; i++) {
        cp_check_margin_step[i] = _cp_check_margin_step[i];
      }
    };
    void set_act_contact_states (const double _act_contact_states[4]) {
      for (size_t i = 0; i < 4; i++) {
    ...
    double get_cp_check_margin_step (const size_t idx) const { return cp_check_margin_step[idx]; };
