...
namespace rats
{
  ...
  public:
    ...
    void set_overwrite_footstep_gain (const double _overwrite_footstep_gain[2]) {
      for (size_t i=0; i<2; i++) {
        overwrite_footstep_gain[i] = _overwrite_footstep_gain[i];
      }
    };
    ...
    double get_current_swing_time(const size_t idx) const { return lcg.get_current_swing_time(idx); };
      double get_current_swing_time_ratio() const { return (static_cast<double>(lcg.get_lcg_count())+1.0)/(footstep_nodes_list[lcg.get_footstep_index()][0].step_time/dt); };
