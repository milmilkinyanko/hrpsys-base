...
void Stabilizer::getActualParametersForST()
{
    ...
    if (st_algorithm != OpenHRP::AutoBalancerService::TPCC) {
        ...
        if (use_act_states && !use_footguided_stabilizer) {
            if (is_walking) {
                ...
                if (!after_walking_interpolator->isEmpty())
                    after_walking_interpolator->clear();
        ...
        if (control_mode == MODE_ST) {
            ...
            if (use_zmp_truncation && !is_walking) {
                ...
                if (!szd->is_inside_support_polygon(tmp_new_refzmp, hrp::Vector3::Zero(), true, std::string(print_str)))
        ...
        if (control_mode == MODE_ST) {
            ...
            for (size_t i = 0; i < stikp.size(); i++) {
                ...
                if (i == 0)
                    f_diff += -1 * sensor_force;
                else
                    f_diff += sensor_force;
                for (size_t j = 0; j < 3; ++j) {
                    if ((!ref_contact_states[i] || !act_contact_states[i]) && fabs(ikp.ref_force(j) - sensor_force(j)) > eefm_swing_damping_force_thre[j])
                        large_swing_f_diff[j] = true;
                    if ((!ref_contact_states[i] || !act_contact_states[i]) && (fabs(ikp.ref_moment(j) - ee_moment(j)) > eefm_swing_damping_moment_thre[j]))
                        large_swing_m_diff[j] = true;
                }
                // Moment limitation
                ...
                {  // Rot
                    ...
                    for (size_t j = 0; j < 3; ++j) {
                        ...
                        if (!eefm_use_swing_damping || !large_swing_m_diff[j])
                            tmp_damping_gain(j) = (1 - transition_smooth_gain) * tmp_damping * 10 + transition_smooth_gain * tmp_damping;
                        else
                            tmp_damping_gain(j) = (1 - transition_smooth_gain) * eefm_swing_rot_damping_gain(j) * 10 + transition_smooth_gain * eefm_swing_rot_damping_gain(j);
                    }
            ...
            if (eefm_use_force_difference_control) {
                ...
                if (ref_contact_states != prev_ref_contact_states)
                    pos_ctrl = (foot_origin_rot.transpose() * prev_act_foot_origin_rot) * pos_ctrl;
                if (eefm_use_swing_damping) {
                    hrp::Vector3 tmp_damping_gain;
                    for (size_t i = 0; i < 3; ++i) {
                        if (!large_swing_f_diff[i])
                            tmp_damping_gain(i) = (1 - transition_smooth_gain) * stikp[0].eefm_pos_damping_gain(i) * 10 + transition_smooth_gain * stikp[0].eefm_pos_damping_gain(i);
                        else
                            tmp_damping_gain(i) = (1 - transition_smooth_gain) * eefm_swing_pos_damping_gain(i) * 10 + transition_smooth_gain * eefm_swing_pos_damping_gain(i);
                    }
    ...
    if (joint_control_mode == OpenHRP::RobotHardwareService::TORQUE && control_mode == MODE_ST)
        setSwingSupportJointServoGains();
...
void Stabilizer::waitSTTransition()
{
    ...
    while (transition_count != 0 || (flag ? !(control_mode == MODE_IDLE || control_mode == MODE_AIR) : false)) {
        usleep(10);
...
void Stabilizer::startStabilizer(void)
{
    ...
    if (joint_control_mode == OpenHRP::RobotHardwareService::TORQUE) {
        std::cerr << "[" << print_str << "] "
                  << "Moved to ST command pose and sync to TORQUE mode" << std::endl;
...
void Stabilizer::calcTorque(const hrp::Matrix33& rot)
{
    ...
    for (size_t j = 0; j < 2; j++)
        contact_p.push_back(m_robot->sensor<hrp::ForceSensor>(stikp[j].sensor_name)->link->p);
    ...
    if (control_mode == MODE_ST) {
        for (size_t j = 0; j < stikp.size(); j++) {
            ...
            // }
            for (size_t i = 0; i < jm.numJoints(); i++)
                jm.joint(i)->u -= tq_from_extft(i);
        }
    }
...
void Stabilizer::calcExternalForce(const hrp::Vector3& cog, const hrp::Vector3& zmp, const hrp::Matrix33& rot)
{
    ...
    for (size_t j = 0; j < stikp.size(); j++) {
        STIKParam& ikp = stikp[j];
        if (on_ground && total_force(2) > 1e-6)
            ikp.ref_force.segment(0, 2) += rot.transpose() * total_force.segment(0, 2) * ikp.ref_force(2) / total_force(2);  // set fx,fy
    }
}
...
void Stabilizer::moveBasePosRotForBodyRPYControl()
{
    ...
    if (DEBUGP || (is_root_rot_limit && loop % 200 == 0)) {
        ...
        if (is_root_rot_limit)
            std::cerr << "[" << print_str << "]   Root rot limit reached!!" << std::endl;
...
void Stabilizer::calcFootOriginCoords(hrp::Vector3& foot_origin_pos, hrp::Matrix33& foot_origin_rot)
{
    ...
    for (size_t i = 0; i < stikp.size(); i++) {
        ...
        leg_c[i].rot(0, 0) = xv1(0);
        leg_c[i].rot(1, 0) = xv1(1);
        leg_c[i].rot(2, 0) = xv1(2);
        leg_c[i].rot(0, 1) = yv1(0);
        leg_c[i].rot(1, 1) = yv1(1);
        leg_c[i].rot(2, 1) = yv1(2);
        leg_c[i].rot(0, 2) = ez(0);
        leg_c[i].rot(1, 2) = ez(1);
        leg_c[i].rot(2, 2) = ez(2);
    }
    ...
    if (on_ground && transition_count == 0 && control_mode == MODE_ST) {
        ...
        for (size_t i = 0; i < stikp.size(); i++) {
            ...
            if (copInfo[i * 3 + 2] > 20.0) {
                ...
                is_cop_outside = is_cop_outside || (!szd->is_inside_foot(tmpcop, stikp[i].ee_name == "lleg", cop_check_margin) || szd->is_front_of_foot(tmpcop, cop_check_margin) || szd->is_rear_of_foot(tmpcop, cop_check_margin));
                if (DEBUGP) {
...
void Stabilizer::calcSwingSupportLimbGain()
{
    ...
    if (DEBUGP) {
        ...
        for (size_t i = 0; i < stikp.size(); i++)
            std::cerr << stikp[i].swing_support_gain << " ";
        std::cerr << "], ref_contact_states = [";
        for (size_t i = 0; i < stikp.size(); i++)
            std::cerr << ref_contact_states[i] << " ";
        std::cerr << "], sstime = [";
        for (size_t i = 0; i < stikp.size(); i++)
            std::cerr << controlSwingSupportTime[i] << " ";
        std::cerr << "], toeheel_ratio = [";
        for (size_t i = 0; i < stikp.size(); i++)
            std::cerr << toeheel_ratio[i] << " ";
        std::cerr << "], support_time = [";
        for (size_t i = 0; i < stikp.size(); i++)
            std::cerr << stikp[i].support_time << " ";
        std::cerr << "]" << std::endl;
    }
}
...
void Stabilizer::calcTPCC()
{
    ...
    for (size_t i = 0; i < stikp.size(); i++) {
        if (max_ik_loop_count < stikp[i].ik_loop_count)
...
void Stabilizer::calcEEForceMomentControl()
{
    ...
    double force_z_r = wrenches.at(0)[2];
    double force_x_r = wrenches.at(0)[0];
    double force_z_l = wrenches.at(1)[2];
    double force_x_l = wrenches.at(1)[0];
    ...
    for (size_t i = 0; i < stikp.size(); i++) {
        if (is_ik_enable[i]) {
            ...
            if (is_feedback_control_enable[i]) {
                ...
                hrp::Vector3 adjust_rpy_pitch = hrp::Vector3::Zero();
                hrp::Vector3 adjust_rpy_yaw = hrp::Vector3::Zero();
                ...
                if (is_adjust_enable) {
                    ...
                    double theta_foot_force = (i == 0 ? std::atan2(force_x_r, force_z_r)
                                                      : std::atan2(force_x_l, force_z_l));
                    // 外れ値の場合
                    ...
                    if (std::abs(thetactrl) > 0.1) {
                        ...
                        adjust_rpy_pitch = adjust_rpy_pitch + hrp::Vector3(0., k * (std::min(clamp, std::max(-clamp, thetactrl))), 0.);
                        //                    rats::rotm3times(tmpR[i], tmpR[i], hrp::rotFromRpy(-adjust_rpy));
                    }

                    //
                    double root_rot_yaw = baseRpy(2) + rpy(2);
                    std::cout << "### yaw: " << root_rot_yaw << std::endl;
                    if (std::abs(root_rot_yaw) > 1e-2) {
                        double k_yaw = 0.03;
                        double clamp = 1.0;  // 大き過ぎたら丸める
                        adjust_rpy_yaw = adjust_rpy_yaw + hrp::Vector3(0., 0., k_yaw * (std::min(clamp, std::max(-clamp, root_rot_yaw))));
                    }
                    ...
                    double kz = 12e-2;
                    double clampx = 13e-2;
                    double clampz = 1.6e-1;
//                    tmpp[i](0) = tmpp[i](0) - kx * (std::min(clampx, std::max(0.0, thetactrl * std::abs(zctrl))));
                    tmpp[i](2) = tmpp[i](2) + (i == 0 ? 0.5 : -0.5) * kz * (std::min(clampz, std::max(-clampz, zctrl)));
                    // ここまで
                } else {
                }
                std::cout << "## ee_d_rpy" << i << ": "
                ...
                std::cout << "## adjust" << i << ": "
                          << adjust_rpy_pitch(1)
                          << ", " << adjust_rpy_yaw(2) << std::endl;

                // TODO: 符号わかんない
                stikp[i].ee_d_foot_rpy = stikp[i].ee_d_foot_rpy - adjust_rpy_pitch - adjust_rpy_yaw;
...
double Stabilizer::calcZctrlFromFootForceDiff(double input)
{
    ...
    const double tau = 0.009;
    // 要計算
    //    const double K = 6e-7;
    //    const double D = 1e-7;
    //    const double C = 1e-8;
    const double K = 6.2894e-7;
    const double D = 3.3079e-7;
    const double C = 1.4207e-8;
    ...
    double zctrl = this->updateShiftRegister(input, u);
    std::cout << "### zctrl: " << zctrl << std::endl;
//    return u;
}

double Stabilizer::updateShiftRegister(double input, double output){
    double DELTA_T = 2e-3;
    ...
    this->m_zctrl_output_sum += output * DELTA_T;
    this->m_zctrl_output_sum_sum += this->m_zctrl_output_sum * DELTA_T;
    return this->m_zctrl_output_sum_sum;
}
