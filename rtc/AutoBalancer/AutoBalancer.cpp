...
void AutoBalancer::stopWheeling ()
{
  std::vector<coordinates> tmp_end_coords_list;
  for (std::vector<string>::iterator it = leg_names.begin(); it != leg_names.end(); it++) {
      if ((*it).find("leg") != std::string::npos) tmp_end_coords_list.push_back(coordinates(ikp[*it].target_p0, ikp[*it].target_r0));
...
bool AutoBalancer::setFootStepsWithWheel(const OpenHRP::AutoBalancerService::FootstepsSequence& fss, const OpenHRP::AutoBalancerService::StepParamsSequence& spss, const OpenHRP::AutoBalancerService::WheelParamsSequence& wpss, CORBA::Long overwrite_fs_idx)
{
  bool ret = true;
  {
    ...
    std::vector<leg_type> initial_support_legs; // dummy
    bool is_valid_gait_type = calc_inital_support_legs(0, initial_support_legs_coords, initial_support_legs, start_ref_coords);
    const size_t wpss_len = wpss.length();
    std::vector<double> x_list(wpss_len);
    std::vector<double> v_max_list(wpss_len);
    std::vector<double> a_max_list(wpss_len);
    for (size_t i = 0; i < wpss_len; i++) {
      x_list[i] = wpss[i].wps[0].goal_pos;
...
void AutoBalancer::waitFootSteps()
{
  ...
  while (gg_is_walking || !transition_interpolator->isEmpty() || gg->is_wheeling)
    usleep(1000);
  usleep(1000);
  gg->set_offset_velocity_param(0,0,0);
}
