...
// {
//     std::cerr << "[" << print_str << "] onInitialize()" << std::endl;
...
//                                  )){
//         std::cerr << "[" << print_str << "]failed to load model[" << prop["model"] << "]" << std::endl;
...
//     readVirtualForceSensorParamFromProperties(m_vfs, m_robot, prop["virtual_force_sensor"], std::string(print_str));
...
//         std::cerr << "[" << print_str << "]   name = " << force_sensor_name << std::endl;
//     }
//     std::cerr << "[" << print_str << "] limbCOPOffset ports (" << npforce << ")" << std::endl;
//     for (unsigned int i=0; i<nforce; ++i) {
//         std::string force_sensor_name = force_sensor_names[i];
...
//         std::cerr << "[" << print_str << "]   name = " << nm << std::endl;
//     }
    ...
    //         ikp.ee_name = ee_name;
            ...
            // //
            // stikp.push_back(ikp);
            // jpe_v.push_back(hrp::JointPathExPtr(new hrp::JointPathEx(m_robot, m_robot->link(ee_base), m_robot->link(ee_target), dt, false, std::string(print_str))));
            // // Fix for toe joint
            ...
            // // std::cerr << "[" << print_str << "] End Effector [" << ee_name << "]" << std::endl;
            // // std::cerr << "[" << print_str << "]   target = " << m_robot->link(ikp.target_name)->name << ", base = " << ee_base << ", sensor_name = " << ikp.sensor_name << std::endl;
            // // std::cerr << "[" << print_str << "]   offset_pos = " << ikp.localp.format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "    [", "]")) << "[m]" << std::endl;
...
// {
//     std::cerr << "[" << print_str<< "] onActivated(" << ec_id << ")" << std::endl;
//     return RTC::RTC_OK;
// }
...
// {
//     Guard guard(m_mutex);
//     std::cerr << "[" << print_str<< "] onDeactivated(" << ec_id << ")" << std::endl;
...
{
    // loop++;
    // // std::cout << print_str<< ": onExecute(" << ec_id << ")" << std::endl;
    ...
    //     }
    // }
    if ( m_robot->numJoints() == qRef.data.length() ) {
        if (is_legged_robot) {
            for ( int i = 0; i < m_robot->numJoints(); i++ ){
                qRef.data[i] = m_robot->joint(i)->q;
...
//         for ( int i = 0; i < m_robot->numJoints(); i++ ){
//             m_robot->joint(i)->q = qCurrent.data[i];
//         }
//         // tempolary
...
//     if (transition_count > 0) {
//         for ( int i = 0; i < m_robot->numJoints(); i++ ){
//             m_robot->joint(i)->q = ( qRef.data[i] - transition_joint_q[i] ) * transition_smooth_gain + transition_joint_q[i];
//         }
//     } else {
//         for ( int i = 0; i < m_robot->numJoints(); i++ ){
//             m_robot->joint(i)->q = qRef.data[i];
//         }
//     }
//     if ( transition_count < 0 ) {
//         transition_count++;
...
//             std::cerr << "[" << print_str << "] [" << m_qRef.tm << "] Move to MODE_IDLE" << std::endl;
...
void Stabilizer::getParameter(OpenHRP::AutoBalancerService::stParam& i_stp)
{
    std::cerr << "[" << print_str << "] getParameter" << std::endl;
    for (size_t i = 0; i < 2; i++) {
    ...
    switch(control_mode) {
    case MODE_IDLE: i_stp.controller_mode = OpenHRP::AutoBalancerService::MODE_IDLE; break;
    case MODE_AIR: i_stp.controller_mode = OpenHRP::AutoBalancerService::MODE_AIR; break;
    case MODE_ST: i_stp.controller_mode = OpenHRP::AutoBalancerService::MODE_ST; break;
    case MODE_SYNC_TO_IDLE: i_stp.controller_mode = OpenHRP::AutoBalancerService::MODE_SYNC_TO_IDLE; break;
    case MODE_SYNC_TO_AIR: i_stp.controller_mode = OpenHRP::AutoBalancerService::MODE_SYNC_TO_AIR; break;
    default: break;
    }
    ...
    for (size_t i = 0; i < jpe_v.size(); i++) {
        OpenHRP::AutoBalancerService::IKLimbParameters& ilp = i_stp.ik_limb_parameters[i];
...
void Stabilizer::setParameter(const OpenHRP::AutoBalancerService::stParam& i_stp)
{
    Guard guard(m_mutex);
    std::cerr << "[" << print_str << "] setParameter" << std::endl;
    for (size_t i = 0; i < 2; i++) {
    ...
    std::cerr << "[" << print_str << "]  TPCC" << std::endl;
    std::cerr << "[" << print_str << "]   k_tpcc_p  = [" << k_tpcc_p[0] << ", " <<  k_tpcc_p[1] << "], k_tpcc_x  = [" << k_tpcc_x[0] << ", " << k_tpcc_x[1] << "], k_brot_p  = [" << k_brot_p[0] << ", " << k_brot_p[1] << "], k_brot_tc = [" << k_brot_tc[0] << ", " << k_brot_tc[1] << "]" << std::endl;
    // for (size_t i = 0; i < 2; i++) {
    ...
    // std::cerr << "[" << print_str << "]  RUNST" << std::endl;
    // std::cerr << "[" << print_str << "]   m_torque_k  = [" << m_torque_k[0] << ", " <<  m_torque_k[1] << "]" << std::endl;
    // std::cerr << "[" << print_str << "]   m_torque_d  = [" << m_torque_d[0] << ", " <<  m_torque_d[1] << "]" << std::endl;
    // std::cerr << "[" << print_str << "]   k_run_b  = [" << k_run_b[0] << ", " <<  k_run_b[1] << "]" << std::endl;
    // std::cerr << "[" << print_str << "]   d_run_b  = [" << d_run_b[0] << ", " <<  d_run_b[1] << "]" << std::endl;
    std::cerr << "[" << print_str << "]  EEFM" << std::endl;
    for (size_t i = 0; i < 2; i++) {
    ...
    if (i_stp.eefm_support_polygon_vertices_sequence.length() != stikp.size()) {
        std::cerr << "[" << print_str << "]   eefm_support_polygon_vertices_sequence cannot be set. Length " << i_stp.eefm_support_polygon_vertices_sequence.length() << " != " << stikp.size() << std::endl;
    } else {
        std::cerr << "[" << print_str << "]   eefm_support_polygon_vertices_sequence set" << std::endl;
        std::vector<std::vector<Eigen::Vector2d> > support_polygon_vec;
        ...
        szd->set_vertices(support_polygon_vec);
        szd->print_vertices(std::string(print_str));
    }
    ...
    if (control_mode == MODE_IDLE) {
        ...
        std::cerr << "[" << print_str << "] cannot change end-effectors except during MODE_IDLE" << std::endl;
    }
    for (std::vector<STIKParam>::const_iterator it = stikp.begin(); it != stikp.end(); it++) {
        std::cerr << "[" << print_str << "]  End Effector [" << it->ee_name << "]" << std::endl;
        std::cerr << "[" << print_str << "]   localpos = " << it->localp.format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "    [", "]")) << "[m]" << std::endl;
        std::cerr << "[" << print_str << "]   localR = " << it->localR.format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", "", "    [", "]")) << std::endl;
    }
    if (i_stp.foot_origin_offset.length () != 2) {
        std::cerr << "[" << print_str << "]   foot_origin_offset cannot be set. Length " << i_stp.foot_origin_offset.length() << " != " << 2 << std::endl;
    } else if (control_mode != MODE_IDLE) {
        std::cerr << "[" << print_str << "]   foot_origin_offset cannot be set. Current control_mode is " << control_mode << std::endl;
    } else {
        for (size_t i = 0; i < i_stp.foot_origin_offset.length(); i++) {
    ...
    std::cerr << "[" << print_str << "]   foot_origin_offset is ";
    for (size_t i = 0; i < 2; i++) {
        std::cerr << foot_origin_offset[i].format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "", "    [", "]"));
    }
    std::cerr << "[m]" << std::endl;
    std::cerr << "[" << print_str << "]   eefm_k1  = [" << eefm_k1[0] << ", " << eefm_k1[1] << "], eefm_k2  = [" << eefm_k2[0] << ", " << eefm_k2[1] << "], eefm_k3  = [" << eefm_k3[0] << ", " << eefm_k3[1] << "]" << std::endl;
    std::cerr << "[" << print_str << "]   eefm_zmp_delay_time_const  = [" << eefm_zmp_delay_time_const[0] << ", " << eefm_zmp_delay_time_const[1] << "][s], eefm_ref_zmp_aux  = [" << ref_zmp_aux(0) << ", " << ref_zmp_aux(1) << "][m]" << std::endl;
    std::cerr << "[" << print_str << "]   eefm_body_attitude_control_gain  = [" << eefm_body_attitude_control_gain[0] << ", " << eefm_body_attitude_control_gain[1] << "], eefm_body_attitude_control_time_const  = [" << eefm_body_attitude_control_time_const[0] << ", " << eefm_body_attitude_control_time_const[1] << "][s]" << std::endl;
    if (is_damping_parameter_ok) {
        for (size_t j = 0; j < stikp.size(); j++) {
            std::cerr << "[" << print_str << "]   [" << stikp[j].ee_name << "] eefm_rot_damping_gain = "
                      ...
                      << "[s]" << std::endl;
            std::cerr << "[" << print_str << "]   [" << stikp[j].ee_name << "] eefm_pos_damping_gain = "
                      ...
                      << "[s]" << std::endl;
            std::cerr << "[" << print_str << "]   [" << stikp[j].ee_name << "] "
        ...
        std::cerr << "[" << print_str << "]   eefm damping parameters cannot be set because of invalid param." << std::endl;
    }
    std::cerr << "[" << print_str << "]   eefm_pos_transition_time = " << eefm_pos_transition_time << "[s], eefm_pos_margin_time = " << eefm_pos_margin_time << "[s] eefm_pos_time_const_swing = " << eefm_pos_time_const_swing << "[s]" << std::endl;
    std::cerr << "[" << print_str << "]   cogvel_cutoff_freq = " << act_cogvel_filter->getCutOffFreq() << "[Hz]" << std::endl;
    szd->print_params(std::string(print_str));
    std::cerr << "[" << print_str << "]   eefm_gravitational_acceleration = " << eefm_gravitational_acceleration << "[m/s^2], eefm_use_force_difference_control = " << (eefm_use_force_difference_control? "true":"false") << ", eefm_use_swing_damping = " << (eefm_use_swing_damping? "true":"false") << std::endl;
    std::cerr << "[" << print_str << "]   eefm_ee_error_cutoff_freq = " << stikp[0].target_ee_diff_p_filter->getCutOffFreq() << "[Hz]" << std::endl;
    std::cerr << "[" << print_str << "]  COMMON" << std::endl;
    if (control_mode == MODE_IDLE) {
        st_algorithm = i_stp.st_algorithm;
        std::cerr << "[" << print_str << "]   st_algorithm changed to [" << getStabilizerAlgorithmString(st_algorithm) << "]" << std::endl;
    } else {
        std::cerr << "[" << print_str << "]   st_algorithm cannot be changed to [" << getStabilizerAlgorithmString(st_algorithm) << "] during MODE_AIR or MODE_ST." << std::endl;
    }
    std::cerr << "[" << print_str << "]   emergency_check_mode changed to [" << (emergency_check_mode == OpenHRP::AutoBalancerService::NO_CHECK?"NO_CHECK": (emergency_check_mode == OpenHRP::AutoBalancerService::COP?"COP":"CP") ) << "]" << std::endl;
    std::cerr << "[" << print_str << "]   transition_time = " << transition_time << "[s]" << std::endl;
    std::cerr << "[" << print_str << "]   cop_check_margin = " << cop_check_margin << "[m], "
    ...
    std::cerr << "[" << print_str << "]   contact_decision_threshold = " << contact_decision_threshold << "[N], detection_time_to_air = " << detection_count_to_air * dt << "[s]" << std::endl;
    std::cerr << "[" << print_str << "]   root_rot_compensation_limit = [" << root_rot_compensation_limit[0] << " " << root_rot_compensation_limit[1] << "][rad]" << std::endl;
    // IK limb parameters
    ...
    if (i_stp.ik_limb_parameters.length() != jpe_v.size()) {
        ...
        std::cerr << "[" << print_str << "]   ik_limb_parameters invalid length! Cannot be set. (input = " << i_stp.ik_limb_parameters.length() << ", desired = " << jpe_v.size() << ")" << std::endl;
    } else {
        for (size_t i = 0; i < jpe_v.size(); i++) {
        ...
        if (is_ik_limb_parameter_valid_length) {
            for (size_t i = 0; i < jpe_v.size(); i++) {
                const OpenHRP::AutoBalancerService::IKLimbParameters& ilp = i_stp.ik_limb_parameters[i];
                std::vector<double> ov;
                ov.resize(jpe_v[i]->numJoints());
                for (size_t j = 0; j < jpe_v[i]->numJoints(); j++) {
    ...
    if (is_ik_limb_parameter_valid_length) {
        ...
        std::cerr << "]" << std::endl;
        std::cerr << "[" << print_str << "]   ik_loop_count = [";
        for (size_t i = 0; i < stikp.size(); i++) {
